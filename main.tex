...
\usepackage[backend=biber, style=gb7714-2015, citestyle=authoryear]{biblatex}
...
\begin{frame}[t]
  ...
  \begin{alertblock}{强调区块}
    \green{绿色}
  \end{alertblock}
  给文本加脚注\footnote{脚注的链接 \url{https://github.com/icgw/ucas-beamer}}
\end{frame}
...
\subsection[第 3 节缩写标题（虽然是缩写也可以很长长长长长长）]{第 3 节主标题}\label{subsec:1-3}
...
\begin{frame}[t]
  \frametitle{引用}
  \begin{theorem}[佚名 2020]\label{thm1}
    \[\text{具体 (Concrete)} = \text{连续 (\alert{Con}tinuous)} + \text{离散 (Dis\alert{crete})}\]
  \end{theorem}
  \vfill
  \begin{proof}
    证明过程.
  \end{proof}
  阅读文献~\autocite{graham1989concrete} 并思考定理~\ref{thm1}.
  \blfootnote{\printbibliography[heading=none,keyword={concrete}]}
\end{frame}

\section[第 2 章缩写主标题也可以很长长长长长长长长长]{第二章主标题}\label{sec:2}
